...
//   const { purchasedCourses, userDetails } = useUserDetails(); // Get user details
...
//   // Check if the user has submitted this exam before
//   // const submitedExams = userDetail?.submitedExams || [];
//   const submitedExams = userDetails?.submitedExams || [];
//   console.log(submitedExams);

//   const accordionData = Array.isArray(examData)
//     ? examData.map((exam) => {
//         // const examAlreadySubmitted = submitedExams.includes(exam._id); // Check if exam is submitted
//         // const examAlreadySubmitted = submitedExams.some(
...
//                           onClick={() => navigate("/not-found")} // Redirect to not found page
//                         >
...
const ParentAccordion = ({ courseDetails }) => {
  const { isDarkTheme } = useTheme();
  const { purchasedCourses, userDetails } = useUserDetails();
  const courseId = courseDetails._id;
  const { examData, error } = useExamsByCourseId(courseId);
  const hasPurchased = purchasedCourses.some(
    (course) => course.course._id === courseId
  ...
  useEffect(() => {
    if (Array.isArray(examData) && submitedExams.length > 0) {
      // Create an object mapping each exam ID to whether it's submitted
      const status = {};
      examData.forEach((exam) => {
        status[exam._id] = submitedExams.includes(exam._id);
      });
      setSubmittedExamsStatus(status); // Update state with submission status for all exams
    }
  }, [examData, submitedExams]); // Re-run effect when examData or submitedExams changes

  const accordionData = Array.isArray(examData)
    ? examData.map((exam) => {
        const examAlreadySubmitted = submittedExamsStatus[exam._id] || false; // Get submission status from state
